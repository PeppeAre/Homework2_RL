...
//Funzione per il punto 1.b (controllo a spazio nullo)
...
// --- Implementazione Vision Control (Punto 2b) --- //
                                                ...
                                                Eigen::MatrixXd K)
{
    ...
    // s_d = [0, 0, 1] (obiettivo: guardare dritto verso il marker)
    Eigen::Vector3d s_d {0, 0, 1};
 
    // P_c_o (Posizione dell'oggetto nel frame camera)
    KDL::Frame aruco_frame = toKDL(aruco_msg->pose);
    Eigen::Vector3d _P_c_o(aruco_frame.p.x(), aruco_frame.p.y(), aruco_frame.p.z());
    // R_c (Rotazione del frame camera)
    KDL::Rotation R_c= aruco_frame.M;
 
    // ||P|| (Norma della posizione)
    double p_norm = (double)_P_c_o.norm();
    p_norm = std::max(p_norm, 1e-3); // Evita divisione per zero
    // std::cout << "p_norm = " << p_norm << std::endl;
 
    // s (vettore di direzione attuale)
    Eigen::Vector3d s(0,0,1);
    if (p_norm > 1e-6) {
        s = _P_c_o / p_norm;
    }
 
    // J_cam (Jacobiana della camera)
    Eigen::MatrixXd J_cam = compute_J_cam(chain_);
 
    // S(s) (Matrice skew-simmetrica)
    Eigen::Matrix3d S = skew(s);
    // std::cout << "S = " << S << std::endl;
 
    // R_c_eigen (Conversione KDL::Rotation -> Eigen::Matrix3d)
    Eigen::Matrix3d R_c_eigen;
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
    ...
    // R (Matrice di trasformazione velocità 6x6)
    Eigen::Matrix<double, 6, 6> R;
    R.setZero();
    R.block<3, 3>(0, 0) = R_c_eigen.transpose();
    R.block<3, 3>(3, 3) = R_c_eigen.transpose();
 
    // L (Matrice di interazione)
    ...
    // Stampa l'errore di direzione
    std::cout << "direction error = " << (s-s_d).norm() << std::endl;
    return q_dot;
}
