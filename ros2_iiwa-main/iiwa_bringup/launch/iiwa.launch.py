...
def generate_launch_description():
    ...
    # Trova il percorso della cartella 'models' dentro 'iiwa_description'
    ...
    # Leggi la variabile d'ambiente esistente GZ_SIM_RESOURCE_PATH
    gz_resource_path = os.environ.get('GZ_SIM_RESOURCE_PATH', '')


    if gz_resource_path:
        new_gz_resource_path = models_path + ':' + gz_resource_path
    else:
        new_gz_resource_path = models_path
        
    # impostiamo la variabile d'ambiente
    set_env_var = SetEnvironmentVariable('GZ_SIM_RESOURCE_PATH', new_gz_resource_path)
    ...
    declared_arguments.append(DeclareLaunchArgument(
        'gz_args',
        default_value=['-r ', iiwa_simulation_world],
    ...
    aruco_ros_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            PathJoinSubstitution([
                FindPackageShare('aruco_ros'), # Trova il pacchetto aruco_ros
                'launch',
                'single.launch.py' # Specifica il launch file
            ])
        ]),
        #
         launch_arguments={
        ...
        # Avviamo solo se stiamo usando la simulazione
        condition=IfCondition(use_sim),
    )
    # === FINE NODO ARUCO ===
    ...
    return LaunchDescription(declared_arguments + [set_env_var] + nodes)
